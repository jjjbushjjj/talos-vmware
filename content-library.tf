# TODO This is now working. Vcenter can't dowload image. Network access issue
# resource "vsphere_content_library" "talos" {
#   name            = "talos"
#   storage_backing = [data.vsphere_datastore.talos.id]
# }

data "vsphere_content_library" "talos" {
  name = "talos"
}
...
data "vsphere_content_library_item" "talos" {
  ...
  type       = "ovf"
  library_id = data.vsphere_content_library.talos.id
}
