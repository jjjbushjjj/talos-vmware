...
export GOVC_DATACENTER=DC-LO
export GOVC_USERNAME='devops_service@vsphere.local'
export GOVC_PASSWORD='VLH!eq3*z$2WEQF^'
export GOVC_INSECURE=1
export GOVC_URL="https://vc.kube-prod.lenta.tech"
export GOVC_DATASTORE=NetAppA700_2_Kudrovo_03_k8s_mng
export GOVC_RESOURCE_POOL='CL-INFRA-HILO/Resources'

# export GOVC_NETWORK='vl302_DEV_10.3.\\[22-23\\]'  # Probably this will work
# export GOVC_URL='https://10.3.21.1'
...
create_node () {
    B64_MACHINE_CONFIG=$(cat ${MACHINE_CONFIG_PATH}| base64 | tr -d '\n')

    MACHINE_NAME=$(basename ${MACHINE_CONFIG_PATH} .yaml)

    exit 0
    govc library.deploy ${CLUSTER_NAME}/talos-${TALOS_VERSION} ${CLUSTER_NAME}-${MACHINE_NAME}
    ...
    if [ -z "${GOVC_NETWORK+x}" ]; then
         echo "GOVC_NETWORK is unset, Set network in vcenter";
         read -n1 -r -p 'Press any key to continue' value
    else
